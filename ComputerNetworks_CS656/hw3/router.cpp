...
/* To keep track of received links */
struct rlc
{
    unsigned int ro;
    unsigned int li;
...
/* To pair links and find out routers of a link */
struct l_ctr
{
    unsigned int linkNo;
    unsigned int cntr;
...
int main(int argc, char** argv)
{
    ...
    /* Graph of the routers, an easy structure for Dijsktra */
    std::vector<router_cost> routerData[NBR_ROUTER];
    /* All found links, unique */
    std::vector<rlc> allLinks;
    /* Same information as alllinks, this one is for easy logging */
    std::vector<link_cost> linkLog[5];
    /* Link checker to see if RIB changes */
    ...
    {
        /* allLinks is equivalent to circuit_DB at this point, so it can be used for the first flood */
    ...
    /* Received LSPDU information */
    int rec_ID;
    int rec_rout;
    int rec_link;
    int rec_cost;
    int rec_path;
    /* Flag to check if the received link is already in the database */
    int flag_1;
    /* Flag to check if the both routers of the links are found */
    int flag_2;

    int linkDataLenght=0;
    /* To hold previous node during RIB update */
    int tempPrev;
    
    /* To figure out if timeout occurs */
    int sret;
    fd_set readfds;
    ...
    {   
        ...
        else
        {
            ...
            if(flag_1==0)
            {
                ...
                for(int i=0; i < links_cntr.size(); ++i)
                {
                    if (links_cntr[i].linkNo == rec_link)
                    {
                        ...
                        if(links_cntr[i].cntr==1)
                        {
                            ...
                            std::cerr << "Control check";
                        
                        break;
                    }

                }
                /* It is totally a new link*/
                ...
                /* Flood the message before logging the RIB according to FAQ */
