# -*- coding: utf-8 -*-
"""
Created on Sat Sep 28 00:43:56 2019
    ...
    if streets:
        ...
        for st in streets:
            if (st[0].lower()==streetName.lower()):
                print("Error: This street is already signed, either change its coordinates by \"c\" or use a new name")
                control5=0
                break
            else:
                continue
...
def removeStreet(coors,idx):
    ...
    if streets:
        ...
        print("Error: Street database is empty")
        
    if control3 == 0:
        print("Error: There is no such street registered as: "+streetName)
...
def graph():
    ...
    if streets:
        ...
        for ee in range (0,len(edges)):
            if ee != (len(edges)-1):
                print("  <"+str(edges[ee][0])+","+str(edges[ee][1])+">,")
            else:
                print("  <"+str(edges[ee][0])+","+str(edges[ee][1])+">")
        print("}")
    else:
        ...
        for idx in range(0,len(inp)):
            if inp[idx] == "a":
                if checkInputA(inp):
                    ...
                    print("Error: Your input format is wrong, please check it")
                control1=1
                break
